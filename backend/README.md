...
本项目包含一个Python后端和一个Next.js前端。请按照以下步骤启动完整的应用程序。
...
请确保你已经安装了 Python 3.7+。然后通过 pip 安装项目所需的依赖：
...
服务将默认在 `http://0.0.0.0:8000` 上启动。请保持此终端窗口的运行状态。

### 2. 启动前端界面

接下来，在另一个终端窗口中，我们来启动前端的Web界面。
...
请确保你已经安装了 Node.js。然后通过 npm 安装项目所需的依赖：
```bash
npm install
```
...
npm run dev
```
服务通常会启动在 `http://localhost:3000`。

### 3. 开始使用

现在，你可以在浏览器中打开 `http://localhost:3000`，你将看到一个命令行输入界面。

在输入框中输入任何你想执行的命令（例如 `ping 8.8.8.8 -c 5` 或 `ls -l`），然后点击“Run”按钮。一个新的标签页将会被创建，并实时流式显示命令的输出。
